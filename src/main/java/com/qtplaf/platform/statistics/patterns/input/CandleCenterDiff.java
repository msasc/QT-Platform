...
/**
 * Center difference (vs previous) factor candle input.
 *
 * @author Miquel Sas
 */
public class CandleCenterDiff extends Candle {
	 ...
	 */
	public CandleCenterDiff(int index, PatternInfo patternInfo) {
	...
	public double getInput(List<Data> datas) {
		...
		String alias = Fields.suffix(Fields.CENTER_DIFF, Fields.Suffix.NRM);
		DataInfo dataInfo = Patterns.getDataInfo(getPatternInfo());
		int i = dataInfo.getOutputIndex(alias);
		double center_diff = data.getValue(i);
		return center_diff;
	}
